...
check.inputs <- function(settings) {
  ...
  if (nrow(inputs) > 0) {
    for(i in 1:nrow(inputs)) {
      ...
      if ("id" %in% names(settings$run$inputs[[tag]])) {
        ...
        if (is.na(file)) {
          PEcAn.logger::logger.error("No file found for", tag, " and id", id, "on host", hostname)
        } else {
          if (is.null(settings$run$inputs[[tag]][['path']])) {
            settings$run$inputs[[tag]]['path'] <- file
          } else if (file != settings$run$inputs[[tag]][['path']]) {
            PEcAn.logger::logger.warn("Input file and id do not match for ", tag)
          }
        }
      ...
      if ("id" %in% names(settings$run$inputs[[tag]])) {
        ...
        if (nrow(formats) >= 1) {
          if (formats[1, 'format_id'] != inputs$format_id[i]) {
            PEcAn.logger::logger.warn("@Format of input", tag, "does not match specified input: ",formats[1, 'format_id'],inputs$format_id[i])
  ...
  if (length(allinputs) > 0) {
    PEcAn.logger::logger.info("Unused inputs found :", paste(allinputs, collapse=" "))
  }
...
check.database <- function(database) {
  ...
  if(is.null(database[["user"]])){
    if (!is.null(database$userid)) {
      ...
      PEcAn.logger::logger.info("'username' in database section should be 'user'")
  ...
  if (!PEcAn.DB::db.exists(params=database, FALSE, table=NA)) {
    PEcAn.logger::logger.severe("Invalid Database Settings : ", unlist(database))
  }
  
  # connected
  PEcAn.logger::logger.info("Successfully connected to database : ", unlist(database))
...
check.bety.version <- function(dbcon) {
  ...
  if (! ("1" %in% versions)) {
    PEcAn.logger::logger.severe("No version 1, how did this database get created?")
  }
  
  # check for specific version
  if (! ("20140617163304" %in% versions)) {
    PEcAn.logger::logger.severe("Missing migration 20140617163304, this associates files with models.")
  }
  if (! ("20140708232320" %in% versions)) {
    PEcAn.logger::logger.severe("Missing migration 20140708232320, this introduces geometry column in sites")
  }
  if (! ("20140729045640" %in% versions)) {
    PEcAn.logger::logger.severe("Missing migration 20140729045640, this introduces modeltypes table")
  }
  if (! ("20151011190026" %in% versions)) {
    PEcAn.logger::logger.severe("Missing migration 20151011190026, this introduces notes and user_id in workflows")
  }
...
check.settings <- function(settings, force=FALSE) {
  ...
  if (is.null(settings$ensemble) && is.null(settings$sensitivity.analysis)) {
    PEcAn.logger::logger.warn("No ensemble or sensitivity analysis specified, no models will be executed!")
  }
  ...
  if(!is.null(settings$meta.analysis)){  
    ...
    if (is.null(settings$meta.analysis$threshold)) {
      ...
      PEcAn.logger::logger.info("Setting meta.analysis threshold to ", settings$meta.analysis$threshold)
    }
    if (is.null(settings$meta.analysis$update)) {
      settings$meta.analysis$update <- 'AUTO'
      PEcAn.logger::logger.info("Setting meta.analysis update to only update if no previous meta analysis was found")
    }
    if ((settings$meta.analysis$update != 'AUTO') && is.na(as.logical(settings$meta.analysis$update))) {
      PEcAn.logger::logger.info("meta.analysis update can only be AUTO/TRUE/FALSE, defaulting to FALSE")
  ...
  if ("qsub" %in% names(settings$host)) {
    ...
    if (is.null(settings$host$qstat)) {
      ...
      PEcAn.logger::logger.info("qstat not specified using default value :", settings$host$qstat)
    }
  }
  ...
  if ("modellauncher" %in% names(settings$host)) {
    if (is.null(settings$host$modellauncher$binary)) {
      ...
      PEcAn.logger::logger.info("binary not specified using default value :", settings$host$modellauncher$binary)
    }
  ...
  if ("job.sh" %in% names(settings$model)) {
    if ("prerun" %in% names(settings$model)) {
      PEcAn.logger::logger.severe("You have both settings$model$job.sh and settings$model$prerun, please combine.")
    }
    PEcAn.logger::logger.info("settings$model$job.sh is deprecated use settings$model$prerun instead.")
  ...
  if ("job.sh" %in% names(settings$host)) {
    if ("prerun" %in% names(settings$host)) {
      PEcAn.logger::logger.severe("You have both settings$host$job.sh and settings$host$prerun, please combine.")
    }
    PEcAn.logger::logger.info("settings$host$job.sh is deprecated use settings$host$prerun instead.")
  ...
  if (!file.exists(settings$modeloutdir) && !dir.create(settings$modeloutdir, recursive=TRUE)) {
    PEcAn.logger::logger.severe("Could not create model out folder", settings$modeloutdir)
  }
  ...
  if (!PEcAn.remote::is.localhost(settings$host)) {
    ...
    PEcAn.logger::logger.info("Using ", settings$host$rundir, "to store runs on remote machine")
    ...
    PEcAn.logger::logger.info("Using ", settings$host$outdir, "to store output on remote machine")
  ...
  if (!is.null(settings$pfts) && (length(settings$pfts) > 0)) {
    for (i in 1:length(settings$pfts)) {
      ...
      if (!"name" %in% names(settings$pfts[i]$pft)) {
        PEcAn.logger::logger.severe("No name specified for pft of index: ", i, ", please specify name")
      }
      if (settings$pfts[i]$pft$name == "") {
        PEcAn.logger::logger.severe("Name specified for pft of index: ", i, " can not be empty.")
      }
      ...
      if (!is.null(dbcon)) {# change to if(class(dbcon) == "PostgreSQLConnection")??
        ...
        if (nrow(x) == 0) {
          PEcAn.logger::logger.severe("Did not find a pft with name ", settings$pfts[i]$pft$name,
                        "\nfor model type", settings$model$type)
        }
        if (nrow(x) > 1) {
          PEcAn.logger::logger.warn("Found multiple entries for pft with name ", settings$pfts[i]$pft$name,
                      "\nfor model type", settings$model$type)
        }
      }
      ...
      if (is.null(settings$pfts[i]$pft$outdir)) {
        ...
        PEcAn.logger::logger.info("Storing pft", settings$pfts[i]$pft$name, "in", settings$pfts[i]$pft$outdir)      
      } else {
        PEcAn.logger::logger.debug("Storing pft", settings$pfts[i]$pft$name, "in", settings$pfts[i]$pft$outdir)      
      }
  ...
  if (!is.null(settings$sensitivity.analysis)) {
    if (is.null(settings$sensitivity.analysis$variable)) {
      if (is.null(settings$ensemble$variable)) {
        PEcAn.logger::logger.severe("No variable specified to compute sensitivity.analysis for.")
      }
      PEcAn.logger::logger.info("Setting sensitivity.analysis variable to the same as ensemble variable [", 
    ...
    if(is.null(settings$sensitivity.analysis$start.year)) {
      if(!is.null(settings$run$start.date)) {
        ...
        PEcAn.logger::logger.info("No start date passed to sensitivity.analysis - using the run date (",
        ...
        PEcAn.logger::logger.info("No start date passed to sensitivity.analysis - using the ensemble date (", 
                    settings$sensitivity.analysis$start.year, ").")
      } else {
        PEcAn.logger::logger.info("No start date passed to sensitivity.analysis, and no default available.")
      }
    }
    ...
    if(is.null(settings$sensitivity.analysis$end.year)) {
      if(!is.null(settings$run$end.date)) {
        ...
        PEcAn.logger::logger.info("No end date passed to sensitivity.analysis - using the run date (", 
        ...
        PEcAn.logger::logger.info("No end date passed to sensitivity.analysis - using the ensemble date (", 
                    settings$sensitivity.analysis$end.year, ").")
      } else {
        PEcAn.logger::logger.info("No end date passed to sensitivity.analysis, and no default available.")
      }
    }
    ...
    if (exists("startdate") && !is.null(settings$sensitivity.analysis$start.year) &&
      ...
      PEcAn.logger::logger.severe("Start year of SA should come after the start.date of the run")
    }
    if (exists("enddate") && !is.null(settings$sensitivity.analysis$end.year) &&
      ...
      PEcAn.logger::logger.severe("End year of SA should come before the end.date of the run")
    }
    if (!is.null(settings$sensitivity.analysis$start.year) && 
      ...
      PEcAn.logger::logger.severe("Start year of SA should come before the end year of the SA")
    }
  }
    ...
    if (is.null(settings$run$site$id)) {
      ...
      if (is.null(settings$run$site$name)) {
        if ((is.null(site$sitename) || site$sitename == "")) {
          ...
          PEcAn.logger::logger.info("Setting site name to ", settings$run$site$name)
        }
      } else if (site$sitename != settings$run$site$name) {
        PEcAn.logger::logger.warn("Specified site name [", settings$run$site$name, "] does not match sitename in database [", site$sitename, "]")
      }
      ...
      if (is.null(settings$run$site$lat)) {
        if ((is.null(site$lat) || site$lat == "")) {
          ...
          PEcAn.logger::logger.info("Setting site lat to ", settings$run$site$lat)
        }
      } else if (as.numeric(site$lat) != as.numeric(settings$run$site$lat)) {
        PEcAn.logger::logger.warn("Specified site lat [", settings$run$site$lat, "] does not match lat in database [", site$lat, "]")
      }
      ...
      if (is.null(settings$run$site$lon)) {
        if ((is.null(site$lon) || site$lon == "")) {
          ...
          PEcAn.logger::logger.info("Setting site lon to ", settings$run$site$lon)
        }
      } else if (as.numeric(site$lon) != as.numeric(settings$run$site$lon)) {
        PEcAn.logger::logger.warn("Specified site lon [", settings$run$site$lon, "] does not match lon in database [", site$lon, "]")
      }
    }
  } else {
    settings$run$site$id <- -1
  ...
  if(!is.null(settings$model)){
    if(!is.null(dbcon)){
      if(!is.null(settings$model$id)){
        if(as.numeric(settings$model$id) >= 0){
          ...
          if(nrow(model) == 0) {
            PEcAn.logger::logger.error("There is no record of model_id = ", settings$model$id, "in database")
          }
        } else {
          model <- list()
        }
        ...
        if(nrow(model) > 1){
          PEcAn.logger::logger.warn("multiple records for", settings$model$name, "returned; using the latest")
          row <- which.max(model$updated_at)
          if (length(row) == 0) row <- nrow(model)
          model <- model[row, ]
        } else if (nrow(model) == 0) {
          PEcAn.logger::logger.warn("Model type", settings$model$type, "not in database")
        }
      } else {
    ...
    if (!is.null(model$id)) {
      if (is.null(settings$model$id) || (settings$model$id == "")) {
        ...
        PEcAn.logger::logger.warn("Model id specified in settings file does not match database.")
      }
    } else {
      if (is.null(settings$model$id) || (settings$model$id == "")) {
        settings$model$id <- -1
    ...
    if (!is.null(model$type)) {
      if (is.null(settings$model$type) || (settings$model$type == "")) {
        ...
        PEcAn.logger::logger.warn("Model type specified in settings file does not match database.")
      }
    }
    if (!is.null(model$revision)) {
      if (is.null(settings$model$revision) || (settings$model$revision == "")) {
        ...
        PEcAn.logger::logger.info("Setting model revision to ", settings$model$revision)
      } else if (settings$model$revision != model$revision) {
        PEcAn.logger::logger.warn("Model revision specified in settings file does not match database.")
      }
    }
    ...
    if (is.null(settings$model$delete.raw) || !is.logical(as.logical(settings$model$delete.raw))) {
      PEcAn.logger::logger.info("Option to delete raw model output not set or not logical. Will keep all model output.")
    ...
    if (!is.null(settings$model$id) && (settings$model$id >= 0)) {
      ...
      if (!is.na(binary)) {
        if (is.null(settings$model$binary)) {
          ...
          PEcAn.logger::logger.info("Setting model binary to ", settings$model$binary)
        } else if (binary != settings$model$binary) {
          PEcAn.logger::logger.warn("Specified binary [", settings$model$binary, "] does not match path in database [", binary, "]")
        }
      }
    } else {
      PEcAn.logger::logger.warn("No model binary sepcified in database for model ", settings$model$type)      
    }
  }
  ...
  if (!is.null(settings$database)) {
    ...
    if (!is.null(settings$database$bety)) {
      ...
      if (is.null(settings$database$bety$write)) {
        PEcAn.logger::logger.info("Writing all runs/configurations to database.")
        settings$database$bety$write <- TRUE
      } else {
        settings$database$bety$write <- as.logical(settings$database$bety$write)
        if (settings$database$bety$write) {
          PEcAn.logger::logger.debug("Writing all runs/configurations to database.")
        } else {
          PEcAn.logger::logger.warn("Will not write runs/configurations to database.")
        }
      }
      ...
      if (settings$database$bety$write && !PEcAn.DB::db.exists(params=settings$database$bety, TRUE, table='users')) {
        PEcAn.logger::logger.severe("Invalid Database Settings : ", unlist(settings$database))
      }
